var rule = {
    title: '毒蛇电影[优]',
    host: 'https://www.dushe9.app',
    url: '/show/fyclass-fyfilter-fypage.html',
    filter_url: '{{fl.类型}}-{{fl.地区}}-{{fl.语言}}-{{fl.年份}}-{{fl.排序}}',
    searchUrl: '/search?k=**&page=fypage',
    searchable: 2,
    quickSearch: 0,
    filterable: 1,
    headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36',
        'Referer': 'https://www.dushe9.app',
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8'
    },
    timeout: 5000,
    class_parse: '#nav-swiper&&.nav-swiper-slide;a&&Text;a&&href;/(\\w+).html',
    cate_exclude: 'Netflix|今日更新|专题列表|排行榜',
    tab_order: ['超清', '蓝光', '极速蓝光'],
    tab_exclude: '4K(高峰不卡)',
    play_parse: true,
    lazy: '',
    limit: 20,
    推荐: '.module-one-row-box&&.module-box-inner&&.module-item;.v-item-title:eq(1)&&Text;*;*;*',
    double: false,
    一级: '.module-box-inner&&.module-item;.v-item-title:eq(1)&&Text;img:eq(1)&&data-original;.v-item-bottom&&span:eq(1)&&Text;a&&href',
    二级: {
        title: '.detail-title&&strong:eq(1)&&Text;.detail-tags&&a:eq(-2)&&Text',
        img: '.detail-pic&&img&&data-original',
        desc: '.detail-info-row-main:eq(-2)&&Text;.detail-tags&&a&&Text;.detail-tags&&a:eq(1)&&Text;.detail-info-row-main:eq(1)&&Text;.detail-info-row-main&&Text',
        content: '.detail-desc&&Text',
 tabs: '.source-item',
        tab_text: '.source-item-label:eq(1)&&Text',
        lists: '.episode-list:eq(#id) a',
    },
    搜索: '.search-result-item;.search-result-item-main&&.title:eq(1)&&Text;*;.tags&&span:eq(-1)&&Text;.search-result-item&&href;.desc&&Text',
    图片替换: 'https://www.dushe9.app=>https://vres.miximixi.me',
    预处理: `js:
        try {
            let html = request(rule.host, { headers: rule.headers });
            let scripts = html.match(/<script[^>]+src=["']([^"']*rdul\.js[^"']*)["']/i);
            if (scripts && scripts.length > 0) {
                let img_url = scripts[0].match(/src=["']([^"']+)["']/)[1];
                if (!img_url.startsWith('http')) img_url = rule.host + img_url;
                let img_js = request(img_url, { headers: rule.headers });
                let img_host = img_js.match(/['"](https?:\/\/[^'"]+)['"]/)[1];
                rule.图片替换 = rule.host + '=>' + img_host;
            }
        } catch (e) {
            console.error('预处理失败:', e);
        }
    `,
    filter: '',
    一级f: `js:
        log('正在动态生成分类过滤条件...');
        let urls = [];
        try {
            let html = request(rule.host + '/', { headers: rule.headers });
            let matches = html.match(/href="\/show\/(\d+)-----1-1\.html"/g);
            if (matches) {
                urls = [...new Set(matches)].map(m => rule.host + m.match(/href="([^"]+)"/)[1]);
            } else {
                urls = [
                    rule.host + '/show/1-----1-1.html',
                    rule.host + '/show/2-----1-1.html',
                    rule.host + '/show/3-----1-1.html',
                    rule.host + '/show/4-----1-1.html',
                    rule.host + '/show/6-----1-1.html'
                ];
            }
        } catch (e) {
            console.error('获取分类URL失败:', e);
            urls = [
                rule.host + '/show/1-----1-1.html',
                rule.host + '/show/2-----1-1.html',
                rule.host + '/show/3-----1-1.html',
                rule.host + '/show/4-----1-1.html',
                rule.host + '/show/6-----1-1.html'
            ];
        }

        let filters = {};
        for (let url of urls.slice(0, 5)) {
            try {
                let fclass = url.match(/show\/(\\d+)-/)[1];
                let html = request(url, { headers: rule.headers });
                let tabs = parseDom(html, '.filter-row');
                let data = [];
                for (let tab of tabs) {
                    let title = parseDom(tab, 'strong&&Text').replace(':', '').trim();
                    let lis = parseDom(tab, 'a');
                    let value = [];
                    for (let li of lis) {
                        let n = parseDom(li, 'a&&Text').trim();
                        let v = '';
                        if (!/全部|地区|类型|排序/.test(n)) {
                            let href = parseDom(li, 'a&&href');
                            try {
                                v = href.match(/-(.*?)(?:1-1|-1)\.html/)[1].replace(/-/g, '');
                                v = decodeURIComponent(v);
                            } catch (e) {
                                v = href;
                            }
                        }
                        value.push({ n: n, v: v });
                    }
                    if (value.length > 0) {
                        data.push({ key: title, name: title, value: value });
                    }
                }
                if (data.length > 0) {
                    filters[fclass] = data;
                }
            } catch (e) {
                console.error('解析分类失败:', url, e);
            }
        }
        VODS = [filters];
    `
};
